        ...
        fs.writeFile(filePath, info, "utf8", (err) => {
          ...
          if (!err) {
            res.json({
              jsonResp: info,
              status: "success",
              message: "Informacija issaugoti pavyko",
            });
          } else {
            res.json({
              status: "failed",
              message: "Nepavyko sukurti failo...",
            });
          }
        });
      });
...
/////////////////////////////////////////////////
//koreguojam inpute jau atvaizduota info ir ja atgal issaugom bei atvaizduojam narsyklej
app.put("/save-request/:id", (req, res) => {
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    ...
    json.forEach((el, index) => {
      if(el.id == id) {
        json[index].round = req.body.round; 
        json[index].date = req.body.date; 
        json[index].loc = req.body.loc; 
        json[index].time = req.body.time; 
        json[index].team1 = req.body.team1; 
        json[index].team2 = req.body.team2; 
      }
    });
...
app.get("/get-match/:id", (req, res) => {
  ...
  fs.readFile(filePath, "utf8", (err, data) => {
    ...
    json.forEach((el, index) => {
      if (el.id == id) {
        //jei, einamojo elemento id = persiustam id
        res.json({
          status: "success",
