...
document.querySelector("#search").addEventListener("click", (event) => {
  ...
  let mode = document.querySelector("#search-form").getAttribute("data-mode");
  let method = "POST";
  let id = "";

  if (mode === "edit") {
    method = "PUT";
    id = document.querySelector("#search-form").getAttribute("data-id");
  }

  
  fetch(`http://localhost:3001/save-request/${id}`, {
    method, //////////////////////// buvo method: "POST"
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ round, date, loc, time, team1, team2 }),
  })
    .then((resp) => resp.json())
    .then((resp) => {
      console.log("save req resp", resp);
  ...
  document.querySelectorAll(".edit").forEach((el) => {
    el.addEventListener("click", (event) => {
      ...
      fetch(`http://localhost:3001/get-match/${id}`)
        .then((resp) => resp.json())
        .then((resp) => {
          console.log("testinimas", resp);
          if (resp.status === "success") {
            
            //padaroma matoma forma
            document.querySelector("#search-form").style.display = "block";
            //prie formos nurodome(sutikrinam), kokia operacija atliksim
            document.querySelector("#search-form").setAttribute("data-mode", "edit");
            //prie formos priskiriamas atributas, kuriame nurodomas redaguojamas 'id'
            document.querySelector("#search-form").setAttribute("data-id", id);

            //formos elementams priskiriamos reiksmes
            document.querySelector('[name="round"]').value = resp.jsonResp.round;
            document.querySelector('[name="date"]').value = resp.jsonResp.date;
            document.querySelector('[name="location"]').value = resp.jsonResp.loc;
            document.querySelector('[name="time"]').value = resp.jsonResp.time;
            document.querySelector('[name="team-1"]').value = resp.jsonResp.team1;
            document.querySelector('[name="team-2"]').value = resp.jsonResp.team2;
            console.log("resp.jsonResp.round", resp.jsonResp.round);
